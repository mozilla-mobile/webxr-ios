...
@implementation Renderer {
    ...
    CVOpenGLESTextureCacheRef _capturedImageTextureCache;
...
- (void)_loadMetal {
    ...
    CVOpenGLESTextureCacheCreate(NULL, NULL, _device, NULL, &_capturedImageTextureCache);
...
- (void)_updateSharedUniformsWithFrame:(ARFrame *)frame {
    ...
    uniforms->projectionMatrix = [frame.camera projectionMatrixForOrientation:_orientation viewportSize:_viewportSize zNear:0.001 zFar:1000];
...
- (void)_updateAnchorsWithFrame:(ARFrame *)frame {
    ...
    NSMutableArray *anchorObjects = [NSMutableArray new];
    
    [[frame anchors] enumerateObjectsUsingBlock:^(ARAnchor * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[ARPlaneAnchor class]] == NO)
        {
            [anchorObjects addObject:obj];
        }
    }];
    
    // Update the anchor uniform buffer with transforms of the current frame's anchors
    NSInteger anchorInstanceCount = MIN(anchorObjects.count, kMaxAnchorInstanceCount);
    ...
    for (NSInteger index = 0; index < anchorInstanceCount; index++) {
        ...
        ARAnchor *anchor = anchorObjects[index + anchorOffset];
...
- (id<MTLTexture>)_createTextureFromPixelBuffer:(CVPixelBufferRef)pixelBuffer pixelFormat:(MTLPixelFormat)pixelFormat planeIndex:(NSInteger)planeIndex {
    ...
    CVReturn status = CVOpenGLESTextureCacheCreateTextureFromImage(NULL, _capturedImageTextureCache, pixelBuffer, NULL, 0, pixelFormat, width, height, 0, 0, planeIndex, &texture);
...
- (void)_updateImagePlaneWithFrame:(ARFrame *)frame {
    ...
    CGAffineTransform displayToCameraTransform = CGAffineTransformInvert([frame displayTransformForOrientation:_orientation viewportSize:_viewportSize]);
